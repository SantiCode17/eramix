...
export async function getAllPosts(city?: string): Promise<HousingPost[]> {
  ...
  const { data } = await apiClient.get("/v1/housing", { params });
  return data.data;
}

export async function getMyPosts(): Promise<HousingPost[]> {
  const { data } = await apiClient.get("/v1/housing/mine");
  return data.data;
}
...
export async function createPost(req: {
  ...
  const { data } = await apiClient.post("/v1/housing", req);
  return data.data;
}

export async function deactivatePost(postId: number): Promise<void> {
  await apiClient.delete(`/v1/housing/${postId}`);
}
